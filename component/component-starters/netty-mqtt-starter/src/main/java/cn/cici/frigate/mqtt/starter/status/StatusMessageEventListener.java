package cn.cici.frigate.mqtt.starter.status;
...
public class StatusMessageEventListener implements MessageEventListener {
    ...
    private Map<String, Object> doGetStatus() {
        ...
        if (busServer != null) {
            ...
            //最后次发送消息时间
            if (busServer.getCountInfo().getLastSend() > 0) {
                resultMap.put("lastSentTime", sdf.format(new Date(busServer.getCountInfo().getLastSend())));
            } else {
            ...
            //发送消息数
            resultMap.put("sentNumber", busServer.getCountInfo().getSendNum().get());
            // 心跳次数
